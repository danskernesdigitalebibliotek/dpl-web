...
packages/service-layer/
  ...
  fbi/                        # internal adapter for FBI, the catalogue
    codegen.yml               # graphql-codegen config
    src/
      documents/              # The .graphql operations this adapter sends
      generated/              # graphql-codegen output — do not edit,
                              #   published as ./fbi/contract for consumers' tests
      mappers/                # Raw FBI -> domain DTOs (zod)
      client.ts               # createFbiClient — internal
      types.ts                # FbiConfig — internal
      index.ts                # internal entry for the adapter
```

`fbi/` is generated by `graphql-codegen` rather than Orval, because FBI is
GraphQL: the contract is the SDL vendored at
`/schemas/graphql/dbc-fbi.fbcms-go.graphql`, the prod host's schema that
`/schemas/README.md` names the canonical one. This package is host-agnostic, so
it generates from the contract rather than from whichever profile a given
consumer queries. Run it with `pnpm codegen:fbi`.

It follows [ADR-003](../../docs/architecture/adr-003-service-layer-validation-boundary.md)
like the other adapters: the generated output is the typed reference for the
contract, not the transport. `client.ts` posts the generated document itself,
and `mappers/` validate the answer with zod — held to the generated operation
type by a compile-time check, so a regenerated document that no longer fits the
schema breaks the build instead of failing in production.

`documentMode: string` keeps the generated documents as plain strings, so the
package needs `graphql` at codegen time only and never ships it to a consumer.

What the adapter is for — the catalogue, not the lending provider, names a
digital material — is
[ADR-004](../../docs/architecture/adr-004-catalogue-names-digital-materials.md).
