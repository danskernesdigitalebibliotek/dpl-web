...
`/schemas/graphql/dbc-fbi.graphql`, the snapshot `/schemas/README.md` describes.
This package is host-agnostic, so it generates from the contract rather than
from whichever profile a given consumer queries. Run it with `pnpm codegen:fbi`.
