...
describe('Events overview month separators', () => {
  ...
  it('renders labelled month separators on the overview', () => {
    ...
    cy.findAllByText(anyMonthLabel).should('have.length.at.least', 1);
    cy.findAllByText(anyMonthLabel).first().should('be.visible');
  });
  ...
  it('keeps the month separator after filtering the list over AJAX', () => {
    ...
    // without its label.
    cy.findByText(monthLabel(separatorEvents.second.start)).should(
      'be.visible',
    );
  });
});
