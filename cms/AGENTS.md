...
Strings are extracted from the codebase by **potx** (see ADR-018; it replaced
potion). Every `t()` / `$this->t()` / `formatPlural()` must include a
`context` argument *inline*; the scanner matches on a flat token stream and
cannot follow variables. The default context is the module machine name.
