...
describe("constructSearchUrlWithFacets", () => {
  ...
  // URLSearchParams is the single encoding authority, so a phrase reaches the
  // CMS as written rather than percent-encoded twice. See docs ADR-013.
  it("encodes the query exactly once, so it round-trips unchanged", () => {
    const url = constructSearchUrlWithFacets({
      searchUrl,
      q: "skov badning",
      facets: []
    ...
    expect(url.searchParams.get("q")).toBe("skov badning");
  });
