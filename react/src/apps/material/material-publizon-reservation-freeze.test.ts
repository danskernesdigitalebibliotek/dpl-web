...
const openEbook = (
  ...
  stubMaterialPageBackends(loanStatus);
  if (!signedIn) givenTheVisitorIsNotSignedIn();
  new MaterialPage(story, "e-bog").visit([]);
...
describe("Material page - the Publizon reservation queue frozen", () => {
  ...
  it("Refuses the reservation and tells the patron why", () => {
    openEbook(materialStory.withClosedPublizonReservations, ON_LOAN);
  ...
  it("Takes the reservation as usual outside the freeze", () => {
    openEbook(materialStory.default, ON_LOAN);
  ...
  it("Still lends a material that is available", () => {
    ...
    openEbook(materialStory.withClosedPublizonReservations, AVAILABLE);
