 ...
 *
 * A Biblio loan from the adapter looks almost exactly like the SDK's checkout -
...
const useWedoBooksCheckout = (
  ...
  const { data: checkout } = useQuery<WedoBooksCheckout | null>({
    queryKey: ["wedobooks", "checkout", loanId],
    ...
    staleTime: Infinity,
    // As in useWedoBooksSdk: a loan the SDK cannot fetch has to say so rather
...
export default useWedoBooksCheckout;
