...
|---|---------------------------------------------------------------------------------------------------------------------------------------------------------|---|
| `graphql/dbc-fbi.graphql` | DBC FBI gateway @ `fbi-api.dbc.dk/fbcms-go` — consumed by `react/`, `go/` and `cms/` (the latter via Sailor in the cli container) | `task refresh:dbc-fbi` |
...
Refreshing `dbc-fbi.graphql` touches all three consumers at once: react's
`task codegen:dbc:gateway`, go's `task codegen:graphql`, and cms's
`task dev:codegen:fbi-graphql` (Sailor, needs the cli container running).
Re-run all three and typecheck each project — upstream drift regularly
adds and removes types that the generated clients export, and TypeScript
code importing a removed type will not be caught by validating the
`.graphql` documents alone.
